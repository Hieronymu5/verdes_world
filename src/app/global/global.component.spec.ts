...
const WORLD_CUP_TEAMS = [
  { id: 'brazil', name: 'Brazil', filename: 'players/brazil.json', city: 'Rio', country: 'Brazil', lat: -22.9, lng: -43.2, colors: ['#FEDD00', '#002776'], clubId: '1', playerCount: 1 },
  { id: 'argentina', name: 'Argentina', filename: 'players/argentina.json', city: 'Buenos Aires', country: 'Argentina', lat: -34.6, lng: -58.4, colors: ['#75AADB', '#FFFFFF'], clubId: '2', playerCount: 1 },
];
...
describe('GlobalComponent', () => {
        ...
        Promise.resolve([
          {
            ...
            position: 'FW',
            clubs: [
              { clubName: 'Old Club', city: 'A', country: 'A', lat: 0, lng: 0, fromYear: 2015, toYear: 2017 },
              { clubName: 'Middle Club', city: 'M', country: 'M', lat: 5, lng: 5, fromYear: 2017, toYear: 2018 },
              { clubName: 'Current Club', city: 'B', country: 'B', lat: 10, lng: 10, fromYear: 2018, toYear: 2024 },
            ],
          },
        ]),
      ),
  ...
  it('should load one dot per player at their current club', async () => {
    component.ngOnInit();
  ...
  it('should show every club overlapping a boundary year', async () => {
    component.ngOnInit();
    await fixture.whenStable();
    ...
    expect(component.dots().length).toBe(4);
    expect(component.dots().some((d) => d.title.includes('Old Club'))).toBe(true);
    expect(component.dots().some((d) => d.title.includes('Middle Club'))).toBe(true);
  });
  ...
  it('should clamp the year range from 2006 to 2026', () => {
    expect(component.minYear).toBe(2006);
        ...
        Promise.resolve([
          {
            ...
            position: 'FW',
            clubs: [
              { clubName: 'Only Club', city: 'A', country: 'A', lat: 0, lng: 0, fromYear: 2020, toYear: 2024 },
            ],
          },
        ]),
      ),
